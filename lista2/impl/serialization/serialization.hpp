...
using AnyStructure = std::variant<
    ...
    std::vector<F2k>,                   
    std::vector<Fpk>,                   
    std::pair<EllipticCurve, std::vector<crypto::curves::Point>>,
    std::pair<EllipticCurveBinary, std::vector<crypto::curves::PointF2k>>
>;

using OperationResult = std::variant<std::monostate, Fp, F2k, Fpk, Point, PointF2k>;
...
public:
    ...
    static std::string serialize_Curve(const EllipticCurve& curve, const std::vector<Point>& points, Base base);
...
private:
    ...
    static std::pair<EllipticCurve, std::vector<Point>> deserialize_Curve(const std::string& raw);
};