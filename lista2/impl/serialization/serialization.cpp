...
std::string Serializer::serialize_Curve(const EllipticCurve& curve, const std::vector<Point>& points, Base base) {
    std::stringstream ss;
...
std::pair<EllipticCurveBinary, std::vector<PointF2k>> Serializer::deserialize_CurveBinary(const std::string& raw) {
    ...
    for(int i=0; i < count; ++i) {
        if (t[6+i*2] == "#") pts.push_back(curve.infinity());
        else pts.emplace_back(F2k(str_to_bint(t[6+i*2], b), poly), F2k(str_to_bint(t[7+i*2], b), poly));
    }
    return {curve, pts};
}

std::pair<EllipticCurve, std::vector<Point>> Serializer::deserialize_Curve(const std::string& raw) {
    auto t = split(raw);
    Base b = (Base)std::stoi(t[0]);
    bInt p = str_to_bint(t[2], b);
    int k = std::stoi(t[3]);
    ...
    EllipticCurve curve(ca, cb);
    int count = std::stoi(t[cur++]);
    std::vector<Point> pts;
    for(int i=0; i<count; ++i) {
        if(t[cur] == "#") { pts.push_back(curve.infinity()); cur+=2; }
        else { Fpk px = get_fpk(); Fpk py = get_fpk(); pts.emplace_back(px, py); }
    }
    return {curve, pts};
}
    ...
    if (auto* pair = std::get_if<std::pair<EllipticCurveBinary, std::vector<PointF2k>>>(&structure)) {
        ...
        for (const auto& t : ops) {
            ...
            else if (t == "+")
            {
                ...
                PointF2k a = s.top(); 
                s.pop();
                s.push(pair->first.add(a, b));
            }
        }
        return s.top();
    }
    if (auto* pair = std::get_if<std::pair<EllipticCurve, std::vector<Point>>>(&structure)) {
        std::stack<Point> s;
        for (const auto& t : ops) {
            if (isdigit(t[0])) s.push(pair->second[std::stoi(t)]);
            else if (t == "+") 
...
std::pair<AnyStructure, OperationResult> Serializer::deserializeWithAction(const std::string& raw) {
    ...
    size_t start = 0;
    switch (type) {
        case T_FP: case T_F2K: start = 4 + std::stoi(tokens[3]); break;
        case T_FPK: {
